...
#Returnerar strängrepresentation av den fundamentala analysen
    ...
    def fundamental_analys(self):
        if self.pe_tal== int and self.pe_tal < 0: #Om inte det inlästa talet redan är i strängformat som negativt ändras det 
            pe_tal_str = "negativt"
        else:
            pe_tal_str = str(self.pe_tal) #För posititva värden på pe_talet har vi strängrepresentation av siffran
...
#Returnerar: True/False
    def __gt__(self,other):
        if self.betavärde > other.betavärde:
            return True
        else:
            return False
...
def las_fundamenta(filnamn1):
    with open(filnamn1,"r") as file:
        try:
            while True:
                namn = file.readline().strip()
                if namn =="": #Kontrollerar om filen är slut och avslutar
                    break
                soliditet = file.readline().strip()
                pe_tal = file.readline().strip()
                ps_tal = file.readline().strip()
                aktier_dict[namn] = Aktie(namn,float(soliditet),(pe_tal),float(ps_tal))
            
        except ValueError or FileNotFoundError: #Två möjligheter av fel behövde hanteras pga ovan
            if ValueError:
                print("Value Error 1")
            else:
    ...
    with open(filnamn2,"r") as file:
        for line in file:
            ...
            try:
                ...
                print("TypeError!2") #Då vet vi att något i filen inte stämmer
            ...
            #Vi kan alltså hämta data för flera av våra behov undertiden vi är inne i filen
            try:
...
def sortera_beta(aktier_dict):
    # Skapa en lista med aktieobjekt och deras betavärden. List comprehension skapar (varje element i vår lista har paret namn,betavärde)
    beta_lista = [(namn, aktie.betavärde) for namn, aktie in aktier_dict.items()]#Vi itererar över en .items lista som är ett "nyckel-värde-par"

    # Sortera listan baserat på betavärde i fallande ordning dvs(reverse=True) med hjälpfunktionen lambda
